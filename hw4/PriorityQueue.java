    ...
    // Section: 03
    // Assignment: 04
    // Description: A modified Priority Queue to put the customers into a queue firstly by their start reservation date's.
    //--------------------------------------------------------------------------------------------------------------------------------
...
class PriorityQueue {
    ...
    public PriorityQueue(int capacity) {                                                //A constructor to create the priority queue with the parameter capacity.
    ...
    //  1)  Start reservation dates
    //  2)  Registration years
    //  3)  Customer IDs
    //--------------------------------------------------------------------------------------------------------------------------------------------

    private int compareCustomers(Customer c1, Customer c2) {
        
        if (c1.getStartReservationDate() != c2.getStartReservationDate()){ return Integer.compare(c1.getStartReservationDate(), c2.getStartReservationDate()); }

        else if (c1.getRegistrationYear() != c2.getRegistrationYear()){ return Integer.compare(c1.getRegistrationYear(), c2.getRegistrationYear()); } 
